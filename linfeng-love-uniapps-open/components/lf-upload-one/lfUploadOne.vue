	...
	<view class="u-upload" v-if="!disabled">
		...
		>
			<view
				v-if="changeable"
				class="u-change-icon"
				@tap.stop="selectFile(true)"
			>
				<view class="u-icon">更换</view>
			</view>
			<!-- <view
		...
		<view style="display: inline-block;width: 100%;" @tap="selectFile(false)" v-if="maxCount > lists.length">
 ...
 * @property {Boolean} changeable 是否显示删除图片的按钮（默认true）
		...
		selectFile(changeImage) {
			...
			chooseFile
				.then(res => {
					if(changeImage){
						that.deleteItem(0);
						lists = JSON.parse(JSON.stringify(that.lists));
					}
					let file = null;
